...
function display_toys(obj_arr) {
	obj_arr.forEach(toy => {
		...
		const all_toys_div = document.querySelector('div#toy-box')
...
function like_toy(e) {
	...
	.catch(function(error){
		failed = true
		alert('ERROR: Could not connect to JSON db.')
	})
	.then(function(){
		if (!failed) e.target.parentElement.children[2].innerText = 'Likes: '+(parseInt(card[2].innerText.split(' ')[1])+1)
	})
}
